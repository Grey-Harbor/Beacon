...
| `BEACON_BROWSER_ORIGIN` | No       | None                         | Exact HTTP(S) browser origin authorized for management mutations, such as `http://localhost:5173`, without a path, query, fragment, or credentials. Configure it when the browser-facing origin differs from the host Beacon receives, including the local Next.js development proxy. In production, only an explicit loopback HTTP origin (`localhost`, `127.0.0.1`, or `::1`) uses a non-`Secure` session cookie; public HTTP origins remain `Secure` and therefore require HTTPS in practice. When unset, Beacon requires the `Origin` host to equal the request `Host`. |
| `NODE_ENV`              | No       | `production`                 | Runtime mode: `development`, `test`, or `production`. When `BEACON_BROWSER_ORIGIN` is unset, production marks browser session cookies `Secure`; development and test do not. The example file selects `development`.                                                                                                                                                                                                                                                                                                                                                        |
