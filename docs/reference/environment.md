...
| `BEACON_BROWSER_ORIGIN` | No       | None                         | Exact HTTP(S) browser origin authorized for management mutations, such as `http://localhost:5173`, without a path, query, fragment, or credentials. Configure it when the browser-facing origin differs from the host Beacon receives, including the local Next.js development proxy. When set, its scheme controls the session-cookie transport flag: HTTPS uses `Secure`; an explicit HTTP origin supports local development. When unset, Beacon requires the `Origin` host to equal the request `Host`. |
